...
const { getLydianLydianPrompt } = require('./lydian-system-prompt');
...
class FirildakAIEngine extends EventEmitter {
  ...
  async processRequest(request) {
    ...
    return await this.concurrentManager.execute(
      async () => {
        try {
    ...
    ); // End of concurrentManager.execute()
  }
  ...
  async selectOptimalProvider(request) {
    ...
    const performanceScores = availableProviders
      .map(provider => ({
        provider,
  ...
  async callAzureOpenAI(request, provider, model) {
    ...
    const secureSystemPrompt = getLydianLydianPrompt();
  ...
  async callAnthropic(request, provider, model) {
    ...
    try {
      ...
      // Fallback to mock response for demo
      return {
        content: 'Merhaba! Ben LyDian AI asistaniyim, size nasil yardimci olabilirim?',
        tokens: 60,
        cost: 0.002,
      };
    }
  ...
  getProviderStatus() {
    ...
    for (const [name, provider] of this.aiProviders) {
      const metrics = this.modelPerformance.get(name) || {
        totalRequests: 0,
        successRate: 1,
        averageResponseTime: 0,
      };
  ...
  async smartRoute(request) {
    ...
    const healthyProviders = Array.from(this.aiProviders.values()).filter(provider => {
      const metrics = this.modelPerformance.get(provider.name);
