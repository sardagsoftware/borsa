...
class APIHealthMonitor extends EventEmitter {
    constructor() {
        ...
        this.apiEndpoints = {
            ...
            'azure-cognitive': process.env.AZURE_COGNITIVE_ENDPOINT || 'https://westeurope.api.cognitive.microsoft.com/',
        ...
        return new Promise((resolve) => {
            try {
                ...
                const timeout = setTimeout(() => {
                    ws.terminate();
                    this.updateWebSocketStatus(name, 'unhealthy', Date.now() - startTime, 'Connection timeout', currentStatus);
                    resolve();
                }, this.timeout);
    ...
    getStatusForAPI() {
        ...
        return {
            status: health.overall === 'healthy' && websockets.overall === 'healthy' ? 'operational' : 'degraded',
            timestamp: new Date().toISOString(),
            services: {
                api: {
