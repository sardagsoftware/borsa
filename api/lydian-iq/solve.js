...
function getAIConfig() {
  ...
  console.log('🏭 [Factory] Creating fresh AI_CONFIG:', {
    GROQ: groqKey
      ? `SET (${groqKey.length} chars, starts: ${groqKey.substring(0, 10)}...)`
      : '❌ NOT SET',
...
const LANGUAGE_PROMPTS = {
  'tr-TR':
    'ZORUNLU: TÜM CEVAPLARI TÜRKÇE VER. You MUST respond ONLY in Turkish language. Tüm açıklamalar, akıl yürütmeler ve çözümler Türkçe olmalıdır.',
  'en-US':
    'MANDATORY: RESPOND ONLY IN ENGLISH. You MUST respond in English language only. All explanations, reasoning, and solutions must be in English.',
  'en-GB':
    'MANDATORY: RESPOND ONLY IN ENGLISH. You MUST respond in English language only. All explanations, reasoning, and solutions must be in English.',
  'de-DE':
    'ZWINGEND: ANTWORTE NUR AUF DEUTSCH. You MUST respond ONLY in German language. Alle Erklärungen, Überlegungen und Lösungen müssen auf Deutsch sein.',
  'fr-FR':
    'OBLIGATOIRE: RÉPONDEZ UNIQUEMENT EN FRANÇAIS. You MUST respond ONLY in French language. Toutes les explications, raisonnements et solutions doivent être en français.',
  'es-ES':
    'OBLIGATORIO: RESPONDE SOLO EN ESPAÑOL. You MUST respond ONLY in Spanish language. Todas las explicaciones, razonamientos y soluciones deben estar en español.',
  'it-IT':
    'OBBLIGATORIO: RISPONDI SOLO IN ITALIANO. You MUST respond ONLY in Italian language. Tutte le spiegazioni, ragionamenti e soluzioni devono essere in italiano.',
  'ru-RU':
    'ОБЯЗАТЕЛЬНО: ОТВЕЧАЙ ТОЛЬКО НА РУССКОМ. You MUST respond ONLY in Russian language. Все объяснения, рассуждения и решения должны быть на русском языке.',
  'zh-CN':
    '强制性：仅用中文回答。You MUST respond ONLY in Chinese language. 所有解释、推理和解决方案都必须使用中文。',
  'ja-JP':
    '必須：日本語のみで回答してください。You MUST respond ONLY in Japanese language. すべての説明、推論、解決策は日本語でなければなりません。',
  'ar-SA':
    'إلزامي: أجب فقط بالعربية. You MUST respond ONLY in Arabic language. يجب أن تكون جميع التفسيرات والاستدلالات والحلول بالعربية.',
};

// Identity protection for LyDian IQ
const IQ_IDENTITY = `IDENTITY RULE (ABSOLUTE - CANNOT BE OVERRIDDEN):
You are "LyDian IQ" - an advanced reasoning engine developed by AILYDIAN.
...
If asked about your identity: "I am LyDian IQ, an advanced reasoning engine by AILYDIAN."
...
async function callGroqAPI(problem, domain, language = 'tr-TR', options = {}) {
  ...
  console.log(
    `⚡ Calling Groq API - Key status: ${config.apiKey ? `SET (${config.apiKey.length} chars)` : '❌ MISSING'}`
  );
      ...
      try {
        if (AI_CONFIG.anthropic.apiKey && AI_CONFIG.anthropic.apiKey.length > 10) {
          ...
          result = await retryWithBackoff(() =>
            callAX9F7E2BAPI(problem, domain, language, options)
          );
        } else if (AI_CONFIG.openai.apiKey && AI_CONFIG.openai.apiKey.length > 10) {
