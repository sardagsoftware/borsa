...
async function translateWithOpenAI(text, targetLang, sourceLang = 'auto') {
  try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a professional translator. Translate text accurately while preserving formatting, tone, and context. Return only the translated text without any explanations.',
          },
          {
            role: 'user',
            content: prompt,
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Azure OpenAI API error: ${response.status} ${response.statusText} - ${JSON.stringify(errorData)}`
      );
    }
