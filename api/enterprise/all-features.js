...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      res.statusCode = 500;
      res.end(JSON.stringify({ error: 'İşlem başarısız. Lütfen tekrar deneyin.' }));
    }
    return;
  }
    ...
    if (feature === 'microsoft365') {
      switch (action) {
        ...
        case 'teams_consultation':
          ...
          break;
        case 'outlook_draft':
          result = await ms365.draftOutlookEmail(
            data.to,
            data.subject,
            data.template,
            data.variables
        ...
        case 'calendar_meeting':
          result = await ms365.scheduleLegalMeeting(
            data.subject,
            data.startTime,
            data.endTime,
            data.attendees,
    ...
    // SLACK/DISCORD BOTS
      ...
      switch (action) {
        ...
        case 'bot_consultation':
          result = await slackDiscord.legalConsultationBot(
            data.platform,
            data.userId,
            data.question
          );
