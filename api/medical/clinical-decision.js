...
const DRUG_INTERACTIONS = [
  {
    ...
    severity: 'Major',
    mechanism:
      "NSAIDs inhibit platelet aggregation and can cause gastric ulceration. Combined with warfarin's anticoagulant effect, this significantly increases bleeding risk.",
    clinicalEffect: '3-fold increased risk of GI bleeding',
    recommendation:
      'Avoid combination. Use acetaminophen for analgesia instead. If NSAID necessary, use lowest dose for shortest duration with gastroprotection (PPI).',
    monitoring:
      'Monitor INR closely (weekly), watch for signs of bleeding (melena, hematuria, ecchymosis)',
  ...
  },
  {
    ...
    severity: 'Major',
    mechanism:
      'ACE inhibitors reduce aldosterone secretion, spironolactone blocks aldosterone receptors. Both mechanisms increase potassium retention, leading to additive hyperkalemia risk.',
    clinicalEffect:
      'Severe hyperkalemia (K+ > 6.0 mEq/L) - risk of cardiac arrhythmias, cardiac arrest',
    recommendation:
      'Use combination only when benefits outweigh risks (e.g., heart failure with reduced EF). Require close monitoring.',
    monitoring:
      'Check potassium and creatinine within 3 days of initiation, then weekly for 1 month, then monthly. Educate patient to avoid high-potassium foods.',
  ...
  },
  {
    ...
    severity: 'Major',
    mechanism:
      'Clarithromycin is a strong CYP3A4 inhibitor. Simvastatin is metabolized by CYP3A4. Inhibition leads to 10-20 fold increase in simvastatin levels, increasing rhabdomyolysis risk.',
    clinicalEffect:
      'Rhabdomyolysis (muscle breakdown), acute kidney injury, life-threatening hyperkalemia',
    recommendation:
      'CONTRAINDICATED. Suspend simvastatin during clarithromycin course. Alternative: use azithromycin (no CYP3A4 interaction) or switch to pravastatin/rosuvastatin (not CYP3A4 substrate).',
  ...
  },
  {
    ...
    severity: 'Major',
    mechanism:
      'IV contrast can cause acute kidney injury. Metformin accumulation in renal failure leads to lactic acidosis (mortality 50%).',
    clinicalEffect: 'Metformin-associated lactic acidosis (MALA)',
    recommendation:
      'Hold metformin 48 hours before and after contrast studies. Check eGFR before resuming. Ensure adequate hydration.',
  ...
  },
  {
    ...
    severity: 'Moderate',
    mechanism:
      'Loop diuretics cause hypokalemia and hypomagnesemia, which increase myocardial sensitivity to digoxin, lowering the threshold for digoxin toxicity.',
    clinicalEffect:
      'Digoxin toxicity (nausea, visual changes, arrhythmias - especially ventricular ectopy)',
    recommendation:
      'Monitor and replace potassium/magnesium. Target K+ 4.0-5.0 mEq/L for patients on digoxin.',
    monitoring:
      'Check digoxin level, potassium, and magnesium monthly. Watch for symptoms of toxicity.',
  ...
  },
  {
    ...
    severity: 'Moderate',
    mechanism:
      'Omeprazole inhibits CYP2C19, the enzyme that converts clopidogrel (prodrug) to its active form. This reduces clopidogrel efficacy.',
    clinicalEffect:
      '25% reduction in platelet inhibition, increased risk of stent thrombosis and cardiovascular events',
    recommendation:
      'Avoid omeprazole/esomeprazole. Use pantoprazole or famotidine instead (no significant CYP2C19 interaction).',
  ...
  },
  {
    ...
    clinicalEffect:
      'Serotonin syndrome - triad of autonomic instability, neuromuscular changes (clonus, hyperreflexia), altered mental status. Can be life-threatening.',
    recommendation:
      'Use combination cautiously. Educate patient on SS symptoms. Consider alternative analgesic (acetaminophen, NSAIDs).',
...
function getTreatmentProtocol(condition, severity, comorbidities, allergies) {
  ...
  if (comorbidities.includes('chronic kidney disease')) {
    warnings.push(
      'RENAL DOSING REQUIRED: Adjust medication doses for CKD (use Cockcroft-Gault eGFR)'
    );
  }

  if (comorbidities.includes('liver disease')) {
    warnings.push(
      'HEPATIC DOSING: Avoid hepatotoxic drugs, reduce metabolism-dependent medications'
    );
  }

  if (allergies.includes('penicillin')) {
    warnings.push(
      'PENICILLIN ALLERGY: Avoid beta-lactam antibiotics, use alternatives (fluoroquinolones, macrolides)'
    );
  }
  ...
  return {
    ...
    followUp:
      'Cardiology follow-up in 1-2 weeks post-discharge. Cardiac rehabilitation referral. Lipid panel in 4-6 weeks.',
    warnings,
    timestamp: new Date().toISOString(),
  };
}
...
function checkDrugInteractions(medications) {
  ...
  if (interactionBreakdown.major > 0) {
    clinicalAction =
      '⚠️ MAJOR INTERACTIONS DETECTED: Review medication list immediately. Consider alternatives or implement intensive monitoring.';
  } else if (interactionBreakdown.moderate > 0) {
    clinicalAction =
      'Moderate interactions present. Implement recommended monitoring and patient education.';
  }
...
router.post('/differential-diagnosis', async (req, res) => {
  ...
  try {
    ...
    if (aiResult && aiResult.success) {
      const localResult = generateDifferentialDiagnosis(
        chiefComplaint,
        symptoms,
        age,
        sex,
    ...
    const result = generateDifferentialDiagnosis(
      chiefComplaint,
      symptoms,
      age,
      sex,
