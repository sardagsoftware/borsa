...
const clinicalInterpretations = {
  ...
  glucose_high: {
    ...
    workup: [
      ...
      'C-peptide',
      'Anti-GAD antibodies (if Type 1 suspected)',
    ],
    urgency: 'moderate',
  },
  troponin_high: {
  ...
  potassium_high: {
    ...
    differentials: [
      'Acute kidney injury',
      'Chronic kidney disease',
      'Medications (ACEi, ARB, spironolactone)',
      'Rhabdomyolysis',
      'Tumor lysis syndrome',
      'Hemolysis (specimen)',
    ],
    workup: [
      'Repeat potassium (rule out hemolysis)',
      'ECG (assess for peaked T waves, QRS widening)',
      'BUN/Creatinine',
      'Urinalysis',
      'Medication review',
    ],
  ...
  alt_ast_high: {
    ...
    workup: [
      'Hepatitis panel (HBsAg, Anti-HCV, Anti-HAV IgM)',
      'Ultrasound abdomen',
      'PT/INR',
      'Albumin',
      'Alcohol history',
  ...
  tsh_high: {
    ...
    workup: [
      ...
      'TPO antibodies',
      'Thyroid ultrasound',
      'Clinical assessment (fatigue, weight gain, cold intolerance)',
    ],
    urgency: 'low',
  },
  wbc_high: {
    ...
    workup: [
      ...
      'CRP/ESR',
      'Peripheral blood smear',
      'Consider bone marrow biopsy if blast cells present',
    ],
    urgency: 'moderate',
  },
  creatinine_high: {
  ...
  try {
    ...
    if (criticalValues.length > 0) {
      recommendations.push(
        '🚨 CRITICAL VALUES DETECTED - Immediate physician notification and intervention required'
      );
    ...
    res.status(500).json({
      success: false,
      error: 'Klinik laboratuvar hatasi. Lutfen tekrar deneyin.',
    });
  }
});
...
router.post('/critical-value-check', async (req, res) => {
  try {
    ...
    for (const [testCode, value] of Object.entries(labResults)) {
      ...
      if (
        (reference.critical_low && value < reference.critical_low) ||
        ...
        criticalAlerts.push({
          ...
          unit: reference.unit,
          criticalThreshold:
            value < reference.critical_low ? reference.critical_low : reference.critical_high,
    ...
    console.error('Clinical lab error:', error.message);
