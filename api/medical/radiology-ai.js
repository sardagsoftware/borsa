...
const radiologyFindings = {
    chest: {
        ...
        abnormal: [
            { finding: 'Pneumonia', pattern: 'Consolidation', location: 'Right lower lobe', severity: 'Moderate' },
            { finding: 'Pleural Effusion', pattern: 'Blunting costophrenic angle', location: 'Left', severity: 'Small' },
            { finding: 'Cardiomegaly', pattern: 'Cardiothoracic ratio >0.5', location: 'Cardiac silhouette', severity: 'Mild' },
            { finding: 'Pulmonary Nodule', pattern: 'Round opacity', location: 'Right upper lobe', severity: 'Suspicious' }
        ]
    },
    brain: {
        normal: ['No acute intracranial abnormality', 'Normal gray-white differentiation', 'No midline shift'],
        abnormal: [
            { finding: 'Ischemic Stroke', pattern: 'Hypodensity', location: 'Left MCA territory', severity: 'Acute' },
            { finding: 'Hemorrhage', pattern: 'Hyperdensity', location: 'Right basal ganglia', severity: 'Moderate' },
            { finding: 'Tumor', pattern: 'Mass effect', location: 'Right frontal lobe', severity: 'Large' }
        ]
    },
    bone: {
        normal: ['No fracture', 'Normal bone alignment', 'No dislocation'],
        abnormal: [
            { finding: 'Fracture', pattern: 'Cortical disruption', location: 'Distal radius', severity: 'Displaced' },
            { finding: 'Dislocation', pattern: 'Joint incongruity', location: 'Shoulder', severity: 'Complete' },
            { finding: 'Osteoporosis', pattern: 'Decreased bone density', location: 'Spine', severity: 'Severe' }
        ]
    },
    abdomen: {
        normal: ['Normal bowel gas pattern', 'No free air', 'Normal solid organs'],
        abnormal: [
            { finding: 'Appendicitis', pattern: 'Appendiceal thickening', location: 'Right lower quadrant', severity: 'Acute' },
            { finding: 'Kidney Stone', pattern: 'Calcification', location: 'Left kidney', severity: '5mm' },
            { finding: 'Bowel Obstruction', pattern: 'Dilated loops', location: 'Small bowel', severity: 'Partial' }
        ]
    }
};
...
const cancerDetectionDatabase = {
    lung: [
        { type: 'Lung Nodule', size: '8mm', location: 'RUL', malignancyRisk: 'Intermediate', followUp: '3 months CT' },
        { type: 'Lung Mass', size: '3.5cm', location: 'LLL', malignancyRisk: 'High', followUp: 'PET-CT + Biopsy' },
        { type: 'Ground Glass Opacity', size: '12mm', location: 'RML', malignancyRisk: 'Low', followUp: '6 months CT' }
    ],
    breast: [
        { type: 'Mass', size: '15mm', location: 'UOQ left', birads: 'BI-RADS 4C', recommendation: 'Biopsy' },
        { type: 'Calcifications', pattern: 'Clustered', location: 'Central right', birads: 'BI-RADS 4A', recommendation: 'Stereotactic biopsy' }
    ],
    brain: [
        { type: 'Glioblastoma', size: '4.2cm', location: 'Right frontal', grade: 'IV', characteristics: 'Ring enhancement, necrosis' },
        { type: 'Meningioma', size: '2.8cm', location: 'Parafalcine', grade: 'I', characteristics: 'Dural tail sign' }
    ],
    liver: [
        { type: 'HCC', size: '3.1cm', location: 'Segment VIII', vascularity: 'Hypervascular', washout: 'Present' },
...
const fractureDetectionDatabase = {
    extremities: [
        { bone: 'Distal Radius', type: 'Colles fracture', displacement: 'Dorsal angulation 20°', comminution: 'Simple' },
        { bone: 'Femoral Neck', type: 'Intracapsular', displacement: 'Garden III', risk: 'AVN risk high' },
        { bone: 'Ankle', type: 'Weber B', stability: 'Unstable', syndesmosis: 'Intact' },
        { bone: 'Scaphoid', type: 'Waist fracture', displacement: 'Non-displaced', risk: 'AVN risk moderate' }
    ],
    spine: [
        { location: 'L1 compression', mechanism: 'Osteoporotic', height_loss: '30%', stability: 'Stable' },
        { location: 'C6 fracture-dislocation', mechanism: 'Trauma', cord: 'Cord compression', stability: 'Unstable' }
    ],
    skull: [
        { location: 'Temporal bone', type: 'Linear fracture', hemorrhage: 'Epidural hematoma', urgency: 'Emergent' }
    ]
};
...
function analyzeMedicalImage(imageType, bodyPart, clinicalHistory) {
    ...
    const hasAbnormality = clinicalHistory && clinicalHistory.toLowerCase().includes('pain') ||
...
function generateRecommendations(hasAbnormality, bodyPart) {
    ...
    const recommendations = {
        chest: ['Recommend CT chest for further evaluation', 'Clinical correlation with labs', 'Follow-up CXR in 6 weeks'],
        brain: ['Neurology consultation', 'MRI brain for detailed evaluation', 'Repeat imaging in 24-48 hours'],
...
function detectCancer(imageType, bodyPart, patientAge, riskFactors) {
    const organCancers = cancerDetectionDatabase[bodyPart.toLowerCase()] || cancerDetectionDatabase.lung;

    // Risk stratification
    const highRisk = riskFactors.some(r =>
...
function detectFracture(imageType, bodyPart, mechanism, clinicalPresentation) {
    const bodyPartKey = bodyPart.toLowerCase().includes('spine') ? 'spine' :
                       bodyPart.toLowerCase().includes('skull') ? 'skull' : 'extremities';
    ...
    const detectedFracture = mechanism.toLowerCase().includes('fall') ? fractures[0] :
                             mechanism.toLowerCase().includes('trauma') ? fractures[1] :
                             fractures[0];
...
function requiresSurgicalConsult(fracture) {
    const surgicalIndicators = ['Unstable', 'Displaced', 'Dislocation', 'Cord compression', 'Emergent'];
    return surgicalIndicators.some(indicator =>
        JSON.stringify(fracture).includes(indicator)
    );
}
...
router.post('/radiologist-assistant', async (req, res) => {
    try {
        ...
        const startTime = Date.now();
        const assistance = await radiologistAIAssistant(imageDescription, clinicalQuestion, patientHistory || {});
...
router.get('/database-stats', (req, res) => {
    res.json({
        ...
        radiologyAI: {
            ...
            bodyParts: ['Chest', 'Brain', 'Spine', 'Abdomen', 'Pelvis', 'Extremities', 'Breast', 'Cardiac'],
