...
class MedicalFileManager {
    ...
    try {
      ...
      const result = await db.query(
        `
                ...
                RETURNING *
            `,
        [sessionId, fileId, userId, analysisType, modelUsed, analysisType, 'pending']
      );
  ...
  async updateAnalysisSession(sessionId, results) {
    ...
    try {
      ...
      const session = await db.query(
        'SELECT user_id FROM medical_analysis_sessions WHERE id = $1',
        [sessionId]
      );
    ...
    try {
      ...
      await db.query(
        `
                ...
                WHERE id = $3
            `,
        [JSON.stringify(deviceData), JSON.stringify(deviceData.dicomTags || {}), fileId]
      );
  ...
  async logFileAccess(fileId, userId, action, metadata = {}) {
    ...
    try {
      ...
      return { success: false, error: 'Dosya erişim günlüğü hatası' };
    }
  }
