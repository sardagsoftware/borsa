...
function getAppointments(filters = {}) {
    ...
    if (filters.date) {
        ...
        filtered = filtered.filter(a =>
            new Date(a.scheduledTime).toDateString() === filterDate
        );
    }
...
function startConsultation(appointmentId, providerId) {
    const appointment = appointments.find(a =>
        a.id === appointmentId && a.providerId === providerId
    );
...
router.post('/schedule-appointment', async (req, res) => {
    try {
        const { patientId, patientName, providerId, consultationType, scheduledTime, reason, symptoms } = req.body;

        if (!patientId || !patientName || !providerId || !consultationType || !scheduledTime || !reason) {
            return res.status(400).json({
                success: false,
                error: 'Missing required fields: patientId, patientName, providerId, consultationType, scheduledTime, reason'
            });
        }
        ...
        console.log(`📅 Appointment scheduled: ${appointment.id} for ${patientName} with ${appointment.provider.name}`);
...
router.get('/waiting-room', (req, res) => {
    try {
        ...
        waitingPatients.sort((a, b) =>
            new Date(a.waitingRoomJoinedAt) - new Date(b.waitingRoomJoinedAt)
        );
...
router.post('/submit-prescription', async (req, res) => {
    try {
        const { consultationId, patientId, patientName, providerId, providerName, medications, diagnosis, refills } = req.body;
