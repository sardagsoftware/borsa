...
const SAMPLE_TEST_RESULTS = {
  PT001: [
    ...
    },
    {
      ...
      criticalValues: [],
      aiInsights:
        'Slight elevation in glucose consistent with diabetes diagnosis. Continue current treatment.',
    },
    {
      id: 'TEST003',
      type: 'VITAL_SIGNS',
  ...
  PT001: [
    {
      ...
      message:
        'Your recent lab results are now available. Please review them in your portal. Overall, everything looks good with a slight elevation in glucose as expected.',
    ...
    },
    {
      ...
      message:
        'I have been experiencing some mild stomach discomfort after taking Metformin. Is this normal?',
    ...
    },
    {
      ...
      message:
        'Mild GI upset is common when starting Metformin. Try taking it with food. If symptoms persist after 2 weeks, please contact me.',
  ...
  PT002: [
    {
      ...
      message:
        'Merhaba Ayşe Hanım, son kontrolünüz çok iyiydi. Migren ataklarınızı azaltmak için düzenli uyku, su tüketimi ve tetikleyicilerden kaçınmaya devam edin.',
...
router.get('/patient-profile', (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: 'Hasta portali islemi basarisiz. Lutfen tekrar deneyin.',
    });
  }
});
  ...
  try {
    ...
    res.json({
      ...
      summary: {
        ...
        activeConditions: medicalRecord.conditions.filter(
          c => c.status === 'Active' || c.status === 'Aktif'
        ).length,
        activeMedications: medicalRecord.medications.filter(
          m => m.status === 'Active' || m.status === 'Aktif'
        ).length,
...
router.get('/appointments', (req, res) => {
  try {
    ...
    const upcoming = appointments.filter(
      a =>
        new Date(a.dateTime) > new Date() && (a.status === 'Scheduled' || a.status === 'Planlandı')
    );
...
router.get('/prescriptions', (req, res) => {
  try {
    ...
    // Filter by status if specified
    if (status) {
      prescriptions = prescriptions.filter(p => p.status.toLowerCase() === status.toLowerCase());
    }

    const active = prescriptions.filter(p => p.status === 'Active' || p.status === 'Aktif');
  ...
  try {
    ...
    const healthSummary = {
      ...
      currentHealth: {
        ...
        criticalAllergies: medicalRecord.allergies.filter(
          a => a.severity === 'CRITICAL' || a.severity === 'KRİTİK'
        ).length,
...
router.get('/export-health-data', (req, res) => {
  try {
    ...
    const exportData = {
      ...
      complianceStatement:
        'This export complies with HIPAA Privacy Rule and Blue Button 2.0 standards',
    };
...
function calculateHealthScore(medicalRecord, testResults) {
  ...
  const activeConditions = medicalRecord.conditions.filter(
    c => c.status === 'Active' || c.status === 'Aktif'
  ).length;
  score -= activeConditions * 5;

  // Deduct for critical allergies
  const criticalAllergies = medicalRecord.allergies.filter(
    a => a.severity === 'CRITICAL' || a.severity === 'KRİTİK'
  ).length;
