...
function calculateTrialMetrics(trial) {
  ...
  const withdrawalRate = ((trial.enrollment.withdrawn / trial.enrollment.randomized) * 100).toFixed(
    1
  );
  ...
  const seriousAERate = ((trial.adverseEvents.serious / trial.enrollment.randomized) * 100).toFixed(
    1
  );

  const sitesActive = trial.sites.filter(s => s.status === 'ACTIVE').length;
  const avgEnrollmentPerSite =
    trial.sites.length > 0 ? (trial.enrollment.current / sitesActive).toFixed(1) : 0;
...
router.post('/trials', async (req, res) => {
  try {
    ...
    const newTrial = {
      ...
      estimatedCompletion: new Date(Date.now() + 730 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split('T')[0],
      sites: [],
      enrollment: {
...
router.post('/adverse-events', async (req, res) => {
  try {
    const { trialId, participantId, siteId, severity, term, onset, relatedness } = req.body;
...
router.get('/analytics', async (req, res) => {
  try {
    ...
    const activeTrials = clinicalTrials.filter(t =>
      ['RECRUITING', 'ACTIVE', 'ENROLLING'].includes(t.status)
    ).length;
