...
const FHIR_PATIENT_DATABASE = {
  ...
  'patient-002': {
    ...
    condition: [{ code: 'J45', display: 'Asthma', onsetDate: '2010-03-12' }],
    medications: [{ name: 'Albuterol Inhaler', dosage: 'As needed', startDate: '2010-04-01' }],
  },
};
...
const DICOM_STUDIES_DATABASE = {
  'study-001': {
    ...
    radiologistReport: {
      ...
      impression:
        'Small 4mm nodule RUL. Recommend 6-month follow-up. No acute cardiopulmonary abnormality.',
      acr_lung_rads: '2',
    },
  },
  'study-002': {
    ...
    radiologistReport: {
      ...
      impression:
        'Normal brain MRI. No acute intracranial abnormality. No evidence of demyelination, infarction, or mass effect.',
  ...
  'vcf-001': {
    ...
    clinicallyRelevantVariants: [
      ...
      },
      {
        ...
        recommendation:
          'Genetic counseling. Consider lifestyle modifications (exercise, Mediterranean diet).',
    ...
    pharmacogenomics: [
      {
        ...
        recommendation:
          'Avoid codeine/tramadol. Consider alternative opioids. Tamoxifen: Monitor therapy.',
...
router.post('/fhir-patient-search', (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: 'Saglik veri muhendisligi hatasi. Lutfen tekrar deneyin.',
    });
  }
});
...
router.post('/genomics-vcf-analysis', (req, res) => {
  try {
    ...
    res.json({
      ...
      recommendation:
        'Refer to genetic counseling for BRCA2 pathogenic variant. Consider cascade testing for family members.',
    });
  } catch (error) {
    res.status(500).json({
      success: false,
  ...
  try {
    res.json({
      ...
      dicomPipeline: {
        ...
        totalSeries: Object.values(DICOM_STUDIES_DATABASE).reduce(
          (sum, s) => sum + s.numberOfSeries,
          0
        ),
        totalImages: Object.values(DICOM_STUDIES_DATABASE).reduce(
      ...
      genomicsProcessing: {
        ...
        totalVariants: Object.values(GENOMICS_VCF_DATABASE).reduce(
          (sum, v) => sum + v.variantStats.totalVariants,
          0
        ),
        pathogenicVariants: Object.values(GENOMICS_VCF_DATABASE).reduce(
          (sum, v) => sum + v.variantStats.clinVarPathogenic,
          0
        ),
