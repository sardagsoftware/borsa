...
const CONDITION_DATABASE = {
  'Acute Coronary Syndrome': {
    ...
    differentials: [
      ...
      { condition: 'Pericarditis', probability: 0.1 },
      { condition: 'GERD', probability: 0.3 },
    ],
  },
  'Community-Acquired Pneumonia': {
    ...
    differentials: [
      { condition: 'Bronchitis', probability: 0.4 },
  ...
  'Type 2 Diabetes Mellitus': {
    ...
    differentials: [
      { condition: 'Type 1 Diabetes', probability: 0.1 },
  ...
  'Bacterial Meningitis': {
    ...
    differentials: [
      { condition: 'Viral Meningitis', probability: 0.6 },
      { condition: 'Subarachnoid Hemorrhage', probability: 0.1 },
  ...
  'Deep Vein Thrombosis': {
    ...
    differentials: [
      ...
      { condition: 'Baker Cyst Rupture', probability: 0.1 },
      { condition: 'Superficial Thrombophlebitis', probability: 0.2 },
...
const DRUG_INTERACTIONS = {
  'Warfarin-NSAID': {
    ...
    recommendation:
      'AVOID combination. Use acetaminophen for analgesia. If NSAIDs necessary, add PPI and monitor INR closely.',
  ...
  'Metformin-Contrast': {
    ...
    recommendation:
      'Hold metformin at time of or before contrast. Resume 48h after if eGFR stable (check eGFR before resuming).',
  ...
  'Simvastatin-Amiodarone': {
    ...
    recommendation:
      'Limit simvastatin to 20mg daily with amiodarone. Consider switching to pravastatin or rosuvastatin (non-CYP3A4).',
  ...
  'SSRI-MAOI': {
    ...
    recommendation:
      'ABSOLUTE CONTRAINDICATION. Requires 2-week washout (5 weeks for fluoxetine) between switching.',
  ...
  'Clarithromycin-Statin': {
    ...
    recommendation:
      'Temporarily stop statin during clarithromycin course (7-14 days). If chronic need, switch to azithromycin (no interaction) and pravastatin/rosuvastatin.',
  ...
  'ACE-Spironolactone': {
    ...
    recommendation:
      'Use with caution. Check baseline K+ and eGFR. Monitor K+ at 1 week, then monthly for 3 months. Reduce risk if eGFR <45.',
...
router.post('/differential-diagnosis', (req, res) => {
  ...
  res.json({
    ...
    topDiagnosis: topDiagnosis
      ...
      : null,
    clinicalPearl:
      topDiagnosis?.category === 'Cardiovascular'
...
router.post('/treatment-protocol', (req, res) => {
  ...
  res.json({
    ...
    warnings:
      warnings.length > 0 ? warnings : ['No specific warnings based on provided comorbidities'],
    references: {
      guideline: protocol.guideline,
      lastUpdated: '2024',
...
router.post('/drug-interactions', (req, res) => {
  ...
  for (const [key, interaction] of Object.entries(DRUG_INTERACTIONS)) {
    const drug1Match = drugNames.some(
      d =>
        interaction.drug1.toLowerCase().includes(d.toLowerCase()) ||
        d.toLowerCase().includes(interaction.drug1.toLowerCase())
    );
    const drug2Match = drugNames.some(
      d =>
        interaction.drug2.toLowerCase().includes(d.toLowerCase()) ||
        d.toLowerCase().includes(interaction.drug2.toLowerCase())
    );
