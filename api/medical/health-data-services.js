...
const AZURE_FHIR_ENDPOINT =
  process.env.AZURE_FHIR_ENDPOINT || 'https://ailydian-health.azurehealthcareapis.com';
...
const SPECIALTY_METRICS = {
  ...
  oncology: {
    ...
    metrics: [
      ...
      {
        key: 'tumor_markers_cea',
        name: 'CEA (Tumor Marker)',
        unit: 'ng/mL',
        normalRange: '<3',
        critical: '>20',
      },
      {
        key: 'tumor_markers_ca125',
        name: 'CA-125',
        unit: 'U/mL',
        normalRange: '<35',
        critical: '>200',
  ...
  pediatrics: {
    ...
    metrics: [
      ...
      {
        key: 'bilirubin_newborn',
        name: 'Bilirubin (Newborn)',
        unit: 'mg/dL',
        normalRange: '<12',
        critical: '>15',
      },
  ...
  psychiatry: {
    ...
    metrics: [
      ...
      {
        key: 'panss_psychosis',
        name: 'PANSS (Psychosis)',
        unit: 'points',
        normalRange: '30',
        critical: '>95',
      },
      {
        key: 'madrs_depression',
        name: 'MADRS Score',
        unit: 'points',
        normalRange: '0-6',
        critical: '>34',
...
async function translateMedicalText(text, targetLanguage) {
  ...
  try {
    ...
    const systemPrompt =
      'You are a medical translator. Translate accurately while preserving medical terminology.';
    ...
    if (language !== 'en') {
      ...
      translatedMetrics = {
        ...
        metrics: await Promise.all(
          metrics.metrics.map(async metric => ({
            ...metric,
...
- Key metrics: ${specialtyData.metrics
      .map(m => m.name)
      .slice(0, 3)
      .join(', ')}
