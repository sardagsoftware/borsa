...
const NOTIFICATION_EVENTS = {
  ...
  VITALS_CRITICAL_VALUE: {
    ...
    severity: 'CRITICAL',
    template:
      'CRITICAL VITAL SIGN: {parameter} = {value} {unit}. Patient requires immediate attention.',
  },
  ...
  VITALS_SEPSIS_WARNING: {
    ...
    severity: 'CRITICAL',
    template:
      'SEPSIS WARNING: {criteria} met. qSOFA score: {score}. Immediate intervention required.',
  },
  ...
  LAB_INTERPRETATION_COMPLETE: {
    ...
    severity: 'INFO',
    template:
      'Lab result interpretation completed. {interpretations} clinical findings identified.',
  },
  ...
  PHARMACY_DRUG_INTERACTION: {
    ...
    severity: 'CRITICAL',
    template:
      'DRUG INTERACTION DETECTED: {drug1} + {drug2}. Severity: {severity}. {recommendation}',
  },
...
function getNotificationStats(userId = 'system') {
  ...
  return {
    ...
    requiresAcknowledgment: userNotifications.filter(
    ...
    byCategory: {
      ...
      clinicalDecision: userNotifications.filter(n => n.categoryCode === 'CLINICAL_DECISION')
        .length,
...
router.get('/notifications', (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: 'Bildirim islemi basarisiz.',
    });
  }
});
