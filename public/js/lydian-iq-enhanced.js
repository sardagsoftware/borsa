/* global window, document, localStorage, URL, Blob, fetch, navigator, alert */
// ========================================
...
class LydianAnalytics {
  ...
  getStats() {
    ...
    this.events
      .filter(e => e.name === 'connector_used')
      .forEach(e => {
  ...
  async load() {
    ...
    this.loadingPromise = fetch('/data/connectors.json')
      ...
      .then(data => {
        ...
        console.log(
          `✅ Loaded ${data.totalConnectors} connectors from ${Object.keys(data.countries).length} countries`
        );
        return data;
      })
      .catch(error => {
