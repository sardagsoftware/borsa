...
class AiLydianChat {
  ...
  setupEventListeners() {
    ...
    if (sidebarToggle) {
      sidebarToggle.addEventListener('click', () => {
        ...
        console.log(
          'Sidebar toggle clicked, collapsed state:',
  ...
  setDocumentAnalysisPrompt() {
    this.triggerFileUpload();
    this.setModelAndPrompt(
      'ai-assistant',
      'Analiz etmek istediğiniz dokümanı yükleyin (PDF, Word, Text dosyaları desteklenir).'
    );
  }
  ...
  displayImageGenerationResult(data) {
    ...
    data.images.forEach((image, index) => {
      ...
      imgElement.style.cssText =
        'max-width: 300px; max-height: 300px; margin: 10px; border-radius: 8px; cursor: pointer;';
  ...
  async showVisionAnalysisDialog() {
    ...
    this.setModelAndPrompt(
      'ai-assistant',
      'Upload an image for comprehensive AI vision analysis including object detection, OCR, and scene understanding.'
    );
  }
  ...
  selectCapability(capability) {
    ...
    if (messageInput && !messageInput.value.trim()) {
      const prompts = {
        ...
        'lydian-vision': 'LyDian AI teknolojileri nasıl çalışır?',
  ...
  async processFile(file) {
    ...
    this.addMessageToChat(
      'user',
      `📎 Dosya yükleniyor: ${file.name} (${this.formatFileSize(file.size)})`
    );
  ...
  handleEnhancedFileUploadResponse(result, file) {
    if (result.success) {
      ...
      this.addMessageToChat(
        'assistant',
        result.error || 'Dosya işlenirken bir hata oluştu.',
        'error'
      );
    }
  }
