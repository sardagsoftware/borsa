...
const PHARMACOGENOMICS_DATABASE = {
    ...
    'CYP2D6': {
        ...
        substrates: [
            'Codeine', 'Tramadol', 'Tamoxifen', 'Venlafaxine',
            'Metoprolol', 'Carvedilol', 'Risperidone', 'Aripiprazole'
        ],
        phenotypes: {
            'Poor Metabolizer (PM)': {
    ...
    'CYP2C19': {
        ...
        substrates: [
            ...
            'Citalopram', 'Escitalopram', 'Voriconazole'
        ],
        phenotypes: {
            'Poor Metabolizer (PM)': {
                ...
                clinicalImpact: 'HIGH',
                recommendations: {
                    'Clopidogrel': 'AVOID - Use alternative P2Y12 inhibitor (prasugrel, ticagrelor). 3x higher MACE risk.',
    ...
    'TPMT': {
        ...
        fdaLabels: ['Azathioprine (FDA Label - TPMT testing recommended)', '6-Mercaptopurine', 'Thioguanine'],
    ...
    'VKORC1': {
        ...
        variants: {
            '-1639G>A (rs9923231)': {
                ...
                clinicalImpact: 'HIGH',
                recommendations: 'Use pharmacogenomic dosing algorithms (e.g., WarfarinDosing.org) combining VKORC1 + CYP2C9 + clinical factors. Reduces time to therapeutic INR by 30%.'
            }
        },
...
router.post('/variant-interpretation', (req, res) => {
    try {
        ...
        if (variantData.gene === 'BRCA1' || variantData.gene === 'BRCA2') {
            ...
            personalizedRisk = {
                breastCancer: patientSex === 'female' ? `${baseBreastRisk}% lifetime risk` : '1-2% lifetime risk (male breast cancer)',
                ...
                familyHistoryImpact: familyHistory === 'positive' ? 'Consistent with hereditary pattern - cascade screening recommended' : 'First identified case - cascade screening critical'
            };
        }
        ...
        res.json({
            ...
            interpretation: {
                ...
                actionable: variantData.clinicalSignificance === 'Pathogenic' || variantData.clinicalSignificance === 'Likely Pathogenic',
                urgency: variantData.penetrance.includes('High') ? 'HIGH - Immediate genetic counseling and cascade screening' : 'MODERATE - Genetic counseling recommended'
            },
            nextSteps: [
...
router.post('/pharmacogenomics', (req, res) => {
    try {
        ...
        if (pgxData.phenotypes) {
            phenotypeData = phenotype ? pgxData.phenotypes[phenotype] : pgxData.phenotypes['Normal Metabolizer (NM)'];
            ...
            medications.forEach(med => {
                if (pgxData.substrates.some(sub => sub.toLowerCase().includes(med.toLowerCase()))) {
                    affectedMedications.push({
                        medication: med,
                        recommendation: phenotypeData && phenotypeData.recommendations && phenotypeData.recommendations[med]
        ...
        res.json({
            ...
            phenotypeData,
            affectedMedications: affectedMedications.length > 0 ? affectedMedications : 'None of the current medications are major substrates',
            fdaLabels: pgxData.fdaLabels,
            clinicalActionability: {
                level: phenotypeData && phenotypeData.clinicalImpact ? phenotypeData.clinicalImpact : 'UNKNOWN',
                action: affectedMedications.length > 0 ? 'Medication review recommended' : 'Monitor for future prescriptions'
            },
...
router.post('/disease-risk', (req, res) => {
    try {
        ...
        res.json({
            ...
            patientAge,
            ageAppropriateScreening: patientAge ? `Based on age ${patientAge}, refer to screening timeline` : 'N/A',
            geneticTesting: {
                recommended: familyHistoryStrength.includes('recommended') || familyHistoryStrength.includes('strongly'),
                genes: diseaseData.genes,
        ...
        console.error('Disease risk assessment error:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});
