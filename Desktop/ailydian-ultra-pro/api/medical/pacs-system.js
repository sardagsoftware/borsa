...
router.get('/studies/:studyUID', (req, res) => {
    try {
        ...
        // Get related series
        const studySeries = Array.from(SERIES.values()).filter(
...
router.get('/worklist', (req, res) => {
    try {
        ...
        if (date) {
            worklists = worklists.filter(w =>
                w.scheduledDateTime.startsWith(date)
            );
        }
        if (status) {
...
router.get('/modalities', (req, res) => {
    try {
        const modalities = Object.values(MODALITIES).map(mod => ({
            ...mod,
            studyCount: Array.from(STUDIES.values()).filter(
                s => s.modality === mod.code
            ).length
        }));
...
router.get('/statistics', (req, res) => {
    try {
        ...
        const stats = {
            ...
            aiAnalysisRate: (studies.filter(s => s.aiAnalysis.performed).length / studies.length * 100).toFixed(1),
