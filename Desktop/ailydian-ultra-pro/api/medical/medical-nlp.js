...
const ICD10_DATABASE = {
    ...
    'diabetic neuropathy': { code: 'E11.42', description: 'Type 2 diabetes mellitus with diabetic polyneuropathy' },
    'diabetic retinopathy': { code: 'E11.319', description: 'Type 2 diabetes mellitus with unspecified diabetic retinopathy' },
    ...
    'acute coronary syndrome': { code: 'I24.9', description: 'Acute ischemic heart disease, unspecified' },
    'myocardial infarction': { code: 'I21.9', description: 'Acute myocardial infarction, unspecified' },
    'stemi': { code: 'I21.3', description: 'ST elevation (STEMI) myocardial infarction of unspecified site' },
...
const CLINICAL_ENTITIES = {
    DISEASE: ['diabetes', 'hypertension', 'covid-19', 'pneumonia', 'asthma', 'copd', 'heart failure', 'atrial fibrillation'],
    MEDICATION: ['metformin', 'lisinopril', 'atorvastatin', 'aspirin', 'insulin', 'warfarin', 'ibuprofen', 'amoxicillin'],
    PROCEDURE: ['ecg', 'chest x-ray', 'ct scan', 'mri', 'echocardiogram', 'colonoscopy', 'endoscopy', 'biopsy'],
    ANATOMY: ['heart', 'lung', 'kidney', 'liver', 'brain', 'chest', 'abdomen', 'head'],
    SYMPTOM: ['chest pain', 'shortness of breath', 'fever', 'cough', 'nausea', 'headache', 'fatigue', 'dizziness'],
    LAB_TEST: ['glucose', 'hba1c', 'creatinine', 'egfr', 'cholesterol', 'triglycerides', 'hemoglobin', 'wbc'],
    VITAL_SIGN: ['blood pressure', 'heart rate', 'temperature', 'respiratory rate', 'oxygen saturation', 'bmi']
};
...
function generateSOAPNote(clinicalText, patientInfo, encounterType) {
    ...
    return {
        subjective: {
            ...
            historyOfPresentIllness: 'Patient is a 62-year-old male presenting with acute onset chest pain that began 2 hours ago. Pain is described as crushing, substernal, 8/10 severity, radiating to left arm. Associated with diaphoresis and nausea. Denies shortness of breath. Patient has history of hypertension and diabetes.',
            reviewOfSystems: {
            ...
            medications: ['Metformin 1000mg PO BID', 'Lisinopril 20mg PO daily', 'Atorvastatin 40mg PO daily', 'Aspirin 81mg PO daily'],
        ...
        objective: {
            ...
            physicalExam: {
                ...
                cardiovascular: 'Tachycardic, regular rhythm, no murmurs/rubs/gallops. Chest pain on palpation negative.',
            ...
            imagingResults: [
                { study: 'ECG', findings: 'ST elevation 2mm in leads II, III, aVF. Suggestive of inferior STEMI.' },
                { study: 'Chest X-ray', findings: 'Normal heart size, no pulmonary edema, no pneumothorax' }
            ]
        },
        assessment: {
            diagnoses: [
        ...
        plan: {
            ...
            medications: [
                ...
                { medication: 'Ticagrelor 90mg PO BID', indication: 'Dual antiplatelet therapy - 12 months post-PCI' },
            ...
            followUp: 'Cardiology follow-up in 1 week post-discharge. Cardiac rehabilitation referral. Repeat lipid panel in 6 weeks.',
            patientEducation: 'Smoking cessation counseling, cardiac diet (low sodium, low saturated fat), daily aspirin compliance, recognition of ACS symptoms (call 911 immediately).',
            referrals: [
    ...
    for (const [keyword, codeInfo] of Object.entries(ICD10_DATABASE)) {
        if (text.includes(keyword)) {
            foundCodes.push({
                ...
                matchedText: keyword,
                confidence: 0.95,
                category: codeInfo.code.charAt(0) === 'E' ? 'Endocrine/Metabolic' :
                          codeInfo.code.charAt(0) === 'I' ? 'Circulatory' :
                          codeInfo.code.charAt(0) === 'J' ? 'Respiratory' :
                          codeInfo.code.charAt(0) === 'N' ? 'Genitourinary' :
                          codeInfo.code.charAt(0) === 'F' ? 'Mental/Behavioral' : 'Other'
            });
        }
    }
