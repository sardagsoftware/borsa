...
export class AdvancedAIEngine {
  ...
  private async initializeModels() {
    try {
      // LSTM Model (8 layers, 256 neurons)
      this.lstmModel = tf.sequential({
        layers: [
          tf.layers.lstm({ units: 256, returnSequences: true, inputShape: [60, 10] }),
      ...
      // Simplified transformer for browser compatibility
      this.transformerModel = tf.sequential({
        layers: [
          tf.layers.dense({ units: 128, activation: 'relu', inputShape: [60, 10] }),
  ...
  async generateSignal(
    ...
    const tensorInput = tf.tensor3d([features]);

    // LSTM Prediction
    const lstmPrediction = this.lstmModel.predict(tensorInput) as tf.Tensor;
    const lstmScores = await lstmPrediction.data();

    // Transformer Prediction
    const transformerPrediction = this.transformerModel.predict(tensorInput) as tf.Tensor;
