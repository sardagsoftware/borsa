...
class AzureApplicationInsightsTransport extends Transport {
  constructor(opts = {}) {
    ...
    try {
      // Setup Application Insights with v3 API (simplified chain)
      // Configure before setup
      const config = appInsights.Configuration || {};
      ...
      appInsights
        ...
        .start(); // Start immediately after basic setup

      appInsightsClient = appInsights.defaultClient;

      // Post-initialization configuration
      if (appInsightsClient && appInsightsClient.config) {
        // Disable console auto-collection (we use Winston)
        appInsightsClient.config.enableAutoCollectConsole = false;
        // Enable live metrics for production
        appInsightsClient.config.enableSendLiveMetrics = (process.env.NODE_ENV === 'production');
      }
