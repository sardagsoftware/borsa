...
async function getUserAnalytics(userId, options = {}) {
    try {
        ...
        while (currentDate <= end) {
            ...
            // Calculate cost (estimate using gpt-4o-mini as average)
            const cost = calculateCost('gpt-4o-mini', dailyStats.tokens.input, dailyStats.tokens.output);
...
async function getAllTimeStats(userId) {
    try {
        ...
        // Calculate cost
        const cost = calculateCost('gpt-4o-mini', stats.tokens.input, stats.tokens.output);
